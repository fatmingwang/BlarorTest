...
namespace BlazorAppWASMStandAlone.ToPdf
{
    ...
    public class PdfGenerator
    {
        ...
        public static Stream GetPDFStream(string title, string body)
        {
            byte[] l_PDFBytes;
            //Create a new document
            Document document = new Document();
            MemoryStream l_FileStream = new MemoryStream();
            //Create a PDF writer
            PdfWriter.GetInstance(document, l_FileStream);
            ...
            Console.WriteLine("Source length: {0}", l_FileStream.Length.ToString());
            l_PDFBytes = l_FileStream.ToArray();
            MemoryStream destination = new MemoryStream(l_PDFBytes);
            return destination;
        }
    }
}
